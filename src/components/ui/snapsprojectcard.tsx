...
interface SnapProjectCardProps {
  ...
  count: number;
  totalsnaps: number;
  // Optional additional snaps data that will be shown when clicked
  additionalSnaps?: {
    image: string;
    title: string;
    author: string;
...
export default function SnapProjectCard({
  ...
  const snapsToShow = additionalSnaps.length > 0 ? additionalSnaps : 
  ...
  return (
    <div className="w-full max-w-[400px]">
      ...
      {showAdditionalSnaps && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={handleShowMore}>
